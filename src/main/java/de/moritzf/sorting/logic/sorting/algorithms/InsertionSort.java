...
import de.moritzf.sorting.logic.sorting.steps.InsertionStep;
...
public class InsertionSort extends SortingAlgorithm {
  ...
  public boolean doStep() {
    ...
    if (lastMemory != -1) {
      ...
      protocol.add(
          new InsertionStep(currentSortedMarker, currentSearchMarker, currentMemory, currentArray));
  ...
  public String step2LaTeX(int chosenStepNumber) {
    ...
    for (int i = 0; i < lastArray.length; i++) {
      ...
      if (chosenStepNumber >= 1
          && lastArray[i] != this.protocol.get(chosenStepNumber - 1).getArray()[i]) {
    ...
    if (chosenStep.getSearchMarker() != -1 && chosenStep.getMemory() != -1) {
      retString += " \\\\ \n";
