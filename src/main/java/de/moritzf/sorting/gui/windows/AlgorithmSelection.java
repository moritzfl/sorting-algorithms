...
/**
 * This class is the one where it all starts. It provides gui that the user sees right after the
 * software is launched. Here the user can select the algorithm which he wants to use and input the
 * array on which it should be used.
 *
 * @author Moritz Floter
 */
public class AlgorithmSelection extends JFrame implements ActionListener {
    ...
    /**
     * The Constant maxValue. Defines the maximum value that one element that is to be sorted can
     * have.
     */
     ...
     */
    JComboBox<SelectionItem> algorithmSelectionBox =
            new JComboBox<SelectionItem>(SelectionItem.values());

    /**
     * The randomArrayGenerationButton. Starts generation of a random array as input for the sorting
     * algorithm.
     */
    ...
    public AlgorithmSelection() {
        ...
        northpanel.add(
                new JLabel("Enter the number sequence that you want to sort:"), BorderLayout.WEST);
        ...
        arrayInputField.setPrompt(
                "Please enter positive numbers separated by comma (z.B.: 1,2,5,4...)");
    ...
    private void handleStart() {
        ...
        if (input == null) {
            JOptionPane.showMessageDialog(
                    this,
                    "You should enter a few numbers first ...",
                    "Layer-8-Error",
            ...
            JOptionPane.showMessageDialog(
                    this,
                    "<html>The array used as input contains more than"
                            + maxCount
    ...
    /**
     * The class for receiving input events in this kind of window (AlgorithmSelection). When an input
     * event occurs, the appropriate method is invoked.
     */
